-- Avatar com 3 modos: foto (recortada), ícone vetorial pré-definido (bichinho)
-- ou iniciais personalizadas digitadas pelo usuário. avatar_url (adicionado
-- na migration anterior) continua guardando a URL quando o modo é "photo";
-- os campos novos guardam qual ícone (modo "icon") ou quais iniciais (modo
-- "initials") foram escolhidos.
...
drop function if exists public.upsert_my_app_user_profile(uuid, text, text, text, text, text, text, text);
...
create or replace function public.upsert_my_app_user_profile(
  ...
  p_avatar_type text default 'initials',
  p_avatar_icon text default '',
  p_avatar_initials text default ''
)
returns table (
  user_id uuid,
  full_name text,
...
begin
  ...
  values (
    ...
    coalesce(p_avatar_type, 'initials'),
    coalesce(p_avatar_icon, ''),
    coalesce(p_avatar_initials, '')
  )
  ...
  returning
    ...
    profile_upsert.avatar_type,
    profile_upsert.avatar_icon,
    profile_upsert.avatar_initials,
...
grant execute on function public.upsert_my_app_user_profile(uuid, text, text, text, text, text, text, text, text, text, text) to anon, authenticated;
