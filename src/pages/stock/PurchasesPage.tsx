      ...
      <Card className="border border-border">
        ...
        <CardContent className="grid grid-cols-1 sm:grid-cols-5 gap-2 items-end">
          ...
          <div>
            ...
            <Input value={unitCost} onChange={(e) => setUnitCost(e.target.value)} className="h-8 text-sm bg-input" />
          </div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mb-3">
            ...
            <div className="flex items-end justify-end">
              <span className="text-sm font-semibold">Subtotal: R$ {subtotal.toFixed(2).replace('.', ',')}</span>
            </div>
          </div>
          {items.length === 0 ? (
              ...
              <TableBody>
                {items.map(it => (
                  <TableRow key={it.itemId}>
                    ...
                    <TableCell className="text-right">R$ {(it.quantity * it.unitCost).toFixed(2).replace('.', ',')}</TableCell>
