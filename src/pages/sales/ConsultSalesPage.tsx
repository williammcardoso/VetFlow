                ...
                <Card key={sale.id} className="p-4 bg-card border border-border">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">Cliente: {mockClients.find(c => c.id === sale.relatedClientId)?.name || "N/A"}</span>
                      {sale.relatedAnimalId && (
                        <>
                          <FaPaw className="h-4 w-4 text-muted-foreground" />
                          <span className="text-sm">Animal: {mockClients.find(c => c.id === sale.relatedClientId)?.animals.find(a => a.id === sale.relatedAnimalId)?.name || "N/A"}</span>
                        </>
                      )}
                    </div>
